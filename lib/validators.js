...
validators.validate = function(fieldType, docField) {
        let val = true;

        if (fieldType.options.validate !== undefined) {

                if (fieldType.options && fieldType.options.validate) {
                        if (typeof fieldType.options.validate === 'object' && fieldType.options.validate.validator) {
                                if (typeof fieldType.options.validate.validator !== 'function') {
                                        return new Error ("Invalid parameter, should be a function");
                                }
                                val = fieldType.options.validate.validator.call(null, docField);
                        }
                        if (typeof fieldType.options.validate === 'function') {
                                val = fieldType.options.validate.call(null, docField);
                        }
                        if (val === false && fieldType.options && (fieldType.options.validate.message||fieldType.options.validate.msg)) {
                                val = fieldType.options.validate.message || fieldType.options.validate.msg;
                        }
                }
                if (val !== true) {
                        val = typeof val === 'string' ? new Error(val) : new Error('Invalid custom validator value');
                }
                return val;
        }
};
