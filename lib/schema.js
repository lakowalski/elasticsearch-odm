...
// function to test built-in validators (all the functions are in lib/validators.json)
 let checkJsonValidators = function(fieldType, path){
       ...
       let docField = dot.pick(path, doc);
       let field = {type: fieldType.type.esType};
	
	
	const validatedmax = Validators.maxlength(fieldType, docField);
	if(validatedmax instanceof Error) {errs.push(new ValidatorError(path, validatedmax))}

	const validatedmin = Validators.minlength(fieldType, docField);
	if(validatedmin instanceof Error) {errs.push(new ValidatorError(path, validatedmin))}
	
	const validatedenum = Validators.checkenum(fieldType, docField);
	if(validatedenum instanceof Error) {errs.push(new ValidatorError(path, validatedenum))}

	const validatedminnum = Validators.minnum(fieldType, docField);
	if(validatedminnum instanceof Error) {errs.push(new ValidatorError(path, validatedminnum))}

	const validatedmaxnum = Validators.maxnum(fieldType, docField);
	if(validatedmaxnum instanceof Error) {errs.push(new ValidatorError(path, validatedmaxnum))}

	const validatedregex = Validators.checkregex(fieldType, docField);
	if(validatedregex instanceof Error) {errs.push(new ValidatorError(path, validatedregex))}	
