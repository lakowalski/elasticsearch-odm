  ...
  let checkLeaf = function (obj, path) {
    _.forOwn(obj, function (v, k) {
      ...
      if (v.type) {
        ...
        // ES2.x: Remove required field if set on schema since ES will throw an "unsupported parameters". required is checked in validate
        if (_.has(field, 'required')) {
          delete field.required;
        }
	
	// We delete these fields after checking if they are respected (check is in checkjsonvalidators)

	if(_.has(field , 'maxlength')){
	  delete field.maxlength;
	}

	if(_.has(field, 'minlength')){
	...
	if(_.has(field, 'match')){
		delete field.match;
	}

	if(_.has(field, 'validate')) {
...
// function to test built-in validators (all the functions are in lib/validators.json)
 let checkJsonValidators = function(fieldType, path){

       let docField = dot.pick(path, doc);
       let field = {
		type: fieldType.type.esType
	};
	
	
	const validatedmax = Validators.maxlength(fieldType, docField);
	if (validatedmax instanceof Error) {
		errs.push(new ValidatorError(path, validatedmax))
	}

	const validatedmin = Validators.minlength(fieldType, docField);
	if (validatedmin instanceof Error) {
		errs.push(new ValidatorError(path, validatedmin))
	}
	
	const validatedenum = Validators.checkenum(fieldType, docField);
	if (validatedenum instanceof Error) {
		errs.push(new ValidatorError(path, validatedenum))
	}

	const validatedminnum = Validators.minnum(fieldType, docField);
	if (validatedminnum instanceof Error) {	
		errs.push(new ValidatorError(path, validatedminnum))
	}

	const validatedmaxnum = Validators.maxnum(fieldType, docField);
	if (validatedmaxnum instanceof Error) {
		errs.push(new ValidatorError(path, validatedmaxnum))
	}

	const validatedregex = Validators.checkregex(fieldType, docField);
	if (validatedregex instanceof Error) {
		errs.push(new ValidatorError(path, validatedregex))
	}	

	const validate = Validators.validate(fieldType, docField);
	if (validate instanceof Error) {
		errs.push(new ValidatorError(path, validate))
	}		
};
  ...
  let checkLeaf = function (obj, path) {
	...
	checkJsonValidators(v, currentPath);
